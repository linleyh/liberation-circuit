...
// the new process will be able to read the transmission in its first cycle.  
 ...
 {
   add_to_build_queue(TEMPLATE_DEFENCE, core_x + cos(angle, 300), core_y + sin(angle, 300), angle, 0, 0);
 ...
 {
   add_to_build_queue(TEMPLATE_DEFENCE, core_x + cos(angle + 2730, 300), core_y + sin(angle + 2730, 300), angle + 2730, 0, 0);
...
// If this is not the main base, build pickets:
 if (i_am_main_base < 5)
 {
   if (picket_count <= 0)
   {
    add_to_build_queue(TEMPLATE_PICKET, core_x + cos(angle, 600), core_y + sin(angle, 600), angle, 0, 0);
    picket_count = 50;
   } 
    else
     picket_count --;
...
// maybe try to build a builder:
 if (process[TM_BUILDER].visible() <= 0) // doesn't exist (friendly processes are always visible if they exist)
 {
   if (builder_count <= 0)
   {
    add_to_build_queue(TEMPLATE_BUILDER, core_x + cos(angle, 600), core_y + sin(angle, 600), angle, 0, 0);
...
// Now try to build escorts:
 ...
 {
  add_to_build_queue(TEMPLATE_ESCORT, core_x + cos(angle, 600), core_y + sin(angle, 600), angle, 0, 0);
  if (free_escort != -1)
...
// finally, consider building a flagship:
 ...
 {
  if (cycles < 5000)
   add_to_build_queue(TEMPLATE_FLAGSHIP1, core_x + cos(angle, 600), core_y + sin(angle, 600), angle, 0, 0);
    else
     add_to_build_queue(TEMPLATE_FLAGSHIP2, core_x + cos(angle, 600), core_y + sin(angle, 600), angle, 0, 0);
