...
// let's build the outpost (roughly) between the well and the centre of the map:
   build_angle = atan2((world_y() / 2) - data_well_y, (world_x() / 2) - data_well_x) - random(1000) + random(1000);
   add_to_build_queue(TEMPLATE_OUTPOST, 
                      data_well_x + cos(build_angle, 700), 
                      data_well_y + sin(build_angle, 700), 
                      build_angle + 4096, 
                      0,
                      0);  
   break;
