...
core_static_hex_B, 0, 
  ...
  {object_downlink, 387, 
    {component_peak, // component 3
      ...
      {object_pulse_xl:auto_att_right, -1442},
    }
  },
  ...
  {object_downlink, -387, 
    {component_peak, // component 6
      ...
      {object_none, 0},
      {object_none, 0},
      {object_pulse_xl:auto_att_left, 1288},
      {object_build, 0},
    }
  },
  {object_downlink, 281, 
...
while(next_message())
{
 ...
 switch(read_message())
 {
  ...
  case MESSAGE_MAIN_BASE:
// Listen for another base's announcement that it is the main base:
   i_am_main_base = 0;
   heard_main_base = 1;
   break;
/*
  ...
  case MESSAGE_UNDER_ATTACK:
   ...
   {
    if (process[TARGET_MESSAGE].distance_less(3000)
     && process[TARGET_MESSAGE].distance_more(800)
     && arc_length(process[TARGET_MESSAGE].target_angle(), opposite_angle) < 1000)
      target_copy(TARGET_BOMBARD, TARGET_MESSAGE);     
   }
   break; 
*/ 
 }
...
/*
// only tries to bombard visible targets.
if (process[TARGET_BOMBARD].visible()
 && process[TARGET_BOMBARD].distance_less(3000)
// && process[TARGET_BOMBARD].distance_more(800)
 && arc_length(process[TARGET_MESSAGE].target_angle(), opposite_angle) < 1000)
{
 auto_att_spike.fire_spike_at(TARGET_BOMBARD, random(20));
}
*/
