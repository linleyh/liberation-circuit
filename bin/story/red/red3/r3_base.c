...
core_static_hex_C, 0, 
    ...
    {component_snub, // component 1
      ...
      {object_downlink, -98, 
        {component_cap, // component 2
          ...
          {object_none, 0},
          {object_none, 0},
          {object_stream_dir:auto_att_right, 0},
        }
      },
      {object_downlink, -229, 
  ...
  {object_downlink, 0, 
    {component_snub, // component 15
      {object_uplink, 0},
      {object_downlink, 229, 
        {component_cap, // component 16
          {object_uplink, 0},
          {object_none, 0},
          {object_none, 0},
      ...
      {object_downlink, -196, 
        {component_cap, // component 18
          {object_uplink, 0},
          {object_repair, 0},
          {object_build, 0},
          {object_downlink, -282, 
            {component_cap, // component 19
              {object_uplink, 0},
              {object_interface, 0},
              {object_interface, 0},
              {object_interface, 0},
      ...
      {object_downlink, 243, 
        {component_cap, // component 20
          {object_downlink, 381, 
            {component_cap, // component 21
              {object_interface, 0},
              {object_interface, 0},
              {object_interface, 0},
              {object_uplink, 0},
...
// if the new process is an escort, send it its position in the flagship formation
  ...
  && build_target_index <= TM_ESCORT_7)
 {
  ...
  if (formation_pos & 1)
  {
   formation_angle = 3200 - formation_pos * 400;
   formation_dist = 700 + formation_pos * 50;
  }
   else
   {
    formation_angle = -3200 + formation_pos * 400;
...
// the new process will be able to read the transmission in its first cycle.  
 ...
 {
   add_to_build_queue(TEMPLATE_DEFENCE, core_x + cos(angle, 400), core_y + sin(angle, 400), angle, 0, 0);
 ...
 {
   add_to_build_queue(TEMPLATE_DEFENCE, core_x + cos(angle + 2730, 400), core_y + sin(angle + 2730, 400), angle + 2730, 0, 0);
...
// Now try building a harvester
 if (process[TM_HARVESTER].visible() <= 0) // doesn't exist (friendly processes are always visible if they exist)
 {
   add_to_build_queue(TEMPLATE_HARVESTER, core_x + cos(build_angle, 400), core_y + sin(build_angle, 400), build_angle, 0, 0);
...
// If this is not the main base, build pickets:
 if (i_am_main_base < 5)
 {
   if (picket_count <= 0)
   {
    add_to_build_queue(TEMPLATE_PICKET, core_x + cos(build_angle, 600), core_y + sin(build_angle, 600), build_angle, 0, 0);
    picket_count = 50;
   } 
    else
     picket_count --;
...
// maybe try to build a builder:
 if (process[TM_BUILDER].visible() <= 0) // doesn't exist (friendly processes are always visible if they exist)
 {
   if (builder_count <= 0)
   {
    add_to_build_queue(TEMPLATE_BUILDER, core_x + cos(build_angle, 600), core_y + sin(build_angle, 600), build_angle, 0, 0);
...
// Now try to build escorts:
 ...
 {
  add_to_build_queue(TEMPLATE_ESCORT, core_x + cos(build_angle, 600), core_y + sin(build_angle, 600), build_angle, 0, 0);
  if (free_escort != -1)
...
// finally, consider building a flagship:
 ...
 {
  if (cycles < 5000)
   add_to_build_queue(TEMPLATE_FLAGSHIP1, core_x + cos(build_angle, 600), core_y + sin(build_angle, 600), build_angle, 0, 0);
    else
     add_to_build_queue(TEMPLATE_FLAGSHIP2, core_x + cos(build_angle, 600), core_y + sin(build_angle, 600), build_angle, 0, 0);
...
if (attacking > 0)
{ 
 if (get_interface_strength() < 2000)
  special_AI(1, 0);
  
int target_to_broadcast;

  if (process[TARGET_FRONT].visible())
   target_to_broadcast = TARGET_FRONT;
    else
    {
     if (process[TARGET_LEFT].visible())
      target_to_broadcast = TARGET_LEFT;
       else
       {
        if (process[TARGET_RIGHT].visible())
         target_to_broadcast = TARGET_RIGHT;
          else
           target_to_broadcast = TARGET_BACK;
       }
    }
  broadcast_count = 50;
  broadcast_target(9000,
                   CHANNEL_HELP,
                   0,
                   target_to_broadcast,
                   MESSAGE_UNDER_ATTACK);
