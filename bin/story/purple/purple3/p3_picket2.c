...
core_pent_B, 4096, 
  ...
  {object_downlink, 0, 
    {component_cap, // component 1
      {object_move:auto_move:auto_retro, -582},
      {object_uplink, 0},
      {object_move:auto_move, 1238},
      {object_move:auto_move, 223},
    }
  },
...
/*  
  ...
  angle_to_leader = process[TARGET_LEADER].target_angle();
  angle_to_leader += 1024;
//  angle_to_leader = process[TARGET_LEADER].target_angle(); // + 4096;
 ...
 case MODE_ATTACK:
  if (!process[TARGET_MAIN].visible())
  {
   ...
   if (process[TARGET_MAIN].distance_less(2500))
   {
    auto_att_spike.fire_spike_xy(attack_x, attack_y);
   }
   bombard_count --;
   if (bombard_count <= 0)
    mode = saved_mode; // give up
   break;
  }
  if (process[TARGET_MAIN].distance_less(2500))
  {
   auto_att_spike.fire_spike_at(TARGET_MAIN, random(process[TARGET_MAIN].get_components_max()));
  }
  auto_move.approach_target(TARGET_MAIN, 0, 2000);
  bombard_count = 100;
  break;
