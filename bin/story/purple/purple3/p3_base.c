    ...
    {component_long6, // component 1
      ...
      {object_downlink, 0, 
        {component_bowl, // component 3
          ...
          {object_pulse_l:auto_att_fwd, 261},
          {object_stream_dir:auto_att_right, -758},
        }
      },
      {object_downlink, 937, 
    ...
    {component_long6, // component 5
      ...
      {object_downlink, 0, 
        {component_bowl, // component 7
          ...
          {object_none, 0},
          {object_stability:auto_stability, 0},
          {object_pulse_l:auto_att_back, 276},
          {object_stream_dir:auto_att_back, -571},
        }
      },
      {object_downlink, 966, 
    ...
    {component_long6, // component 9
      ...
      {object_downlink, 0, 
        {component_bowl, // component 11
          ...
          {object_none, 0},
          {object_stability:auto_stability, 0},
          {object_pulse_l:auto_att_left, 623},
          {object_stream_dir:auto_att_fwd, -330},
        }
      },
      {object_downlink, 923, 
...
// if the new process is an escort, send it its position in the flagship formation
  ...
  && build_target_index <= TM_ESCORT_7)
 {
  ...
  if (formation_pos & 1)
  {
   ...
   formation_dist = 400 + formation_pos * 50;
  }
   else
   {
    formation_angle = -3200 + formation_pos * 400;
...
// If this is not the main base, build pickets:
 if (i_am_main_base < 5)
 {
   if (picket_count <= 0)
   {
    if (get_process_count() < 30
     || get_available_data() > 1000)
    {
     add_to_build_queue(TEMPLATE_PICKET + random(2), core_x + cos(angle, 400), core_y + sin(angle, 400), angle, 0, 0);
     picket_count = 300;
    } 
   } 
    else
...
// finally, consider building a flagship:
 ...
 {
  if (cycles < 3000)
   add_to_build_queue(TEMPLATE_FLAGSHIP1, core_x + cos(angle, 400), core_y + sin(angle, 400), angle, 0, 0);
    else
