...
core_pent_B, 4096, 
  ...
  {object_downlink, -837, 
    {component_bowl, // component 1
      {object_uplink, 0},
      {object_interface, 0},
      {object_move:auto_move:auto_retro, -341},
      {object_move:auto_move, 30},
      {object_move:auto_move, -1564},
    }
  },
  {object_downlink, 0, 
    {component_fork, // component 2
      {object_none, 0},
      {object_uplink, 0},
      {object_burst_xl:auto_att_main, -1040},
    }
  },
  {object_downlink, 0, 
    {component_fork, // component 3
      {object_none, 0},
      {object_burst_xl:auto_att_main, 1040},
      {object_uplink, 0},
    }
...
//  angle_to_leader = atan2(process[TARGET_PARENT].get_core_y() - core_y, process[TARGET_PARENT].get_core_x() - core_x);
 ...
 case MODE_FORMATION:
  ...
  angle_to_leader = process[TARGET_LEADER].target_angle();
  angle_to_leader += 1024;
  move_x = process[TARGET_LEADER].get_core_x() - cos(angle_to_leader, 2100);
  move_y = process[TARGET_LEADER].get_core_y() - sin(angle_to_leader, 2100);
  auto_move.move_to(move_x, move_y);
  gosub scan_for_any_target;
...
//   using_main_attack = 1;
  ...
  auto_move.approach_track(TARGET_MAIN, random(process[TARGET_MAIN].get_components_max()), auto_att_main, 600);
  break;
