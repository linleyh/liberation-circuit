...
core_pent_A, 0, 
  ...
  {object_downlink, 49, 
    {component_cap, // component 3
      {object_move:auto_move:outer_move, -687},
      {object_uplink, 0},
      {object_downlink, 656, 
        {component_cap, // component 4
          {object_slice:auto_att_fwd, -363},
          {object_move:auto_move:auto_retro, -1378},
          {object_none, 0},
          {object_uplink, 0},
        }
      },
      {object_move:auto_move:auto_retro, 174},
    }
  },
...
if (cycles & 1)
 auto_att_fwd.attack_scan(0, 600, TARGET_FWD);
...
// MODE_GUARD is like MODE_FOLLOW but the process will follow a new leader if one is built
 ...
 case MODE_ATTACK: // attacking something it found itself
  ...
  auto_move.approach_track(TARGET_MAIN, 0, auto_att_fwd, 800);
//  if ((cycles & 1)
//   && process[TARGET_MAIN].distance() < 1000
