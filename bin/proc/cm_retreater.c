#process "retreater"// This process has objects with the following auto classes:
class auto_move;
class auto_att_fwd;
...
core_quad_B, 4096, 
  ...
  {object_downlink, 512, 
    {component_tri, // component 1
      {object_move:auto_move, -1576},
      {object_uplink, -2048},
      {object_pulse:auto_att_fwd, -780},
    }
  },
  {object_downlink, -512, 
    {component_tri, // component 2
      {object_move:auto_move, 1576},
      {object_pulse:auto_att_right, 780},
      {object_uplink, 0},
    }
  },
#code
...
enum
{
  ...
  TARGET_RETREAT, // the target where the process retreats to at 50% health
};
  ...
  switch(get_command_type()) // get_command_type() returns the type of command given
  {
    ...
    case COM_FRIEND:
      get_command_target(TARGET_GUARD); // writes the target of the command to address TARGET_GUARD in targetting memory
       // (targetting memory stores the target and allows the process to examine it if it's in scanning range)
      if (get_command_ctrl() && process[TARGET_GUARD].get_core_x() == get_core_x() && process[TARGET_GUARD].get_core_y())
      {
        if(verbose) printf("\nTerminating.");
        terminate;
      }
...
{
  if (
    get_total_integrity() * 2 < get_unharmed_integrity_max() || // less than 50 percent overall health
    component[0].get_integrity() * 2 < component[0].get_integrity_max() // less than 50 percent core health
  )
  {
    mode = MODE_MOVE;
    move_x = process[TARGET_PARENT].get_core_x();
    move_y = process[TARGET_PARENT].get_core_y();
    bubblef("Retreating to (%i, %i)!", move_x, move_y);
  }
}
