...
static void add_orange_area_details(void);
...
//map_init.background_depth_random_freq = 0;
 ...
 for (i = 0; i < w.players; i ++)
	...
	if (map_init.data_well_style == AREA_ORANGE)
		add_orange_area_details();
   ...
   block_node_circle(block_x, block_y, 3, 1, 1, 2,
    ...
    clear_background_circle(block_x,
     ...
     block_node_circle(block_x, block_y, 3, 3, 2, 4,
																			    ...
																			    BNC_INNER_LINE,
																			    BNC_MIDDLE_LINE,
																			    BNC_OUTER_RANDOM_GRADIENT);	break;
				case 4:
...
#define ORANGE_ADDITIONS 70

 int additions = 0;
 int addition_x [ORANGE_ADDITIONS];
 int addition_y [ORANGE_ADDITIONS];
...
//							&& backbl->node_depth [nd] == -1)
						...
						{
							if (backbl->node_size [nd] < 10)
								backbl->node_depth [nd] = 3;
							  else
									{
							   if (backbl->node_size [nd] < 14)
								   backbl->node_depth [nd] = 2;
								    else
												{
							      if (backbl->node_size [nd] < 19)
								      backbl->node_depth [nd] = 1;
								       else
